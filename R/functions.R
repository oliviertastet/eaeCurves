...
plot.mean<-function(data,days=1:263,min.score=0){
	...
	g=ggplot2::ggplot(a,aes(Group.3, x,fill=Group.2))+geom_point(stroke=1,size=3,shape=21,position = position_jitterdodge(jitter.width=.1))+geom_boxplot(size=1,alpha=.6,outlier.shape=NA)+theme_bw()+scale_fill_manual(values=c('firebrick3','grey80'))+ylab(paste0('Mean clinical score: days ',days[1],'-',days[length(days)]))+ggtitle(title)+xlab('')
	 return(g)
}
